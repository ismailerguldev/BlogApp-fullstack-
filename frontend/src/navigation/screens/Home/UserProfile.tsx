...
import { incFollowing } from '@/src/redux/slices/userSlice';
type Props = StaticScreenProps<{
    user_id: string;
}>;
function UserProfile({ route }: Props) {
    ...
    const getPosts = async () => {
        ...
        try {
            ...
            const pageSize = 5
            const res = await fetch(`http://192.168.1.76:5000/post/user/${page}/${pageSize}?user_id=${user_id}`, {
                method: "GET",
                headers: {
    ...
    const getUser = async () => {
        try {
            const token = await getToken()
            await fetch(`http://192.168.1.76:5000/user/get/${route.params.user_id}`, {
                method: "GET",
                headers: {
                    "Content-Type": "application/json",
                    "Authorization": `Bearer ${token}`
    ...
    const handleFollow = async () => {
        try {
            const token = await getToken()
            await fetch(`http://192.168.1.76:5000/user/${user_id}/follow`, {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                    "Authorization": `Bearer ${token}`
                ...
                if (data.followInfo.message === "FOLLOW") {
                    dispatch(incFollowing(1))
                } else if (data.followInfo.message === "UNFOLLOW") {
                    dispatch(incFollowing(-1))
                }
            })
        } catch (error) {
                    ...
                    <VStack style={{ alignItems: "center", gap: 20, padding: 15, paddingTop: top * 1.4, }}>
                        ...
                        <HStack className='w-full' style={{ justifyContent: "space-around", padding: 15, backgroundColor: "#27292d", borderRadius: 15, }}>
                            <VStack style={{ justifyContent: "center", alignItems: "center" }}>
                                <Text style={{ color: "white", fontSize: 16, }}>{userInfo?.totalPost}</Text>
                                <Text style={{ color: "white", fontSize: 16, }}>Posts</Text>
                            </VStack>
                            <VStack style={{ justifyContent: "center", alignItems: "center" }}>
                                <Text style={{ color: "white", fontSize: 16, }}>{userInfo?.followers}</Text>
                                <Text style={{ color: "white", fontSize: 16, }}>Followers</Text>
                            </VStack>
                            <VStack style={{ justifyContent: "center", alignItems: "center" }}>
                                <Text style={{ color: "white", fontSize: 16, }}>{userInfo?.followings}</Text>
                                <Text style={{ color: "white", fontSize: 16, }}>Followings</Text>
                            </VStack>
                        </HStack>
                        <TouchableOpacity onPress={handleFollow} style={{ width: width * 0.6, justifyContent: "center", alignItems: "center", backgroundColor: "#2E2E2E", borderRadius: 5, alignSelf: "center", padding: 15 }}>
                            <Text style={{ color: "white" }}>Handle Follow</Text>
                        </TouchableOpacity>
                        <Divider />
                    </VStack>
                </>
